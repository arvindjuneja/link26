...
ios/
  ...
  SentryCore/                          ← local Swift Package. ZERO remote deps → no
    ...
    Sources/SentryCore/
      ...
      Engine/    TraceOps.swift · Grading.swift · ShiftOps.swift · Scoring.swift             [C3]
                 (R8: `TraceOps`, not `Trace` — one target cannot hold two `Trace.swift`)
      Career/    CareerOps.swift · Inbox.swift · Templating.swift                            [C4]
      Session/   SessionState.swift · SessionReducer.swift · SocAction.swift · Phase.swift
                 Effect.swift                                          (as shipped, C11)     [C5]
      Feel/      SocCue.swift · HapticPattern.swift · CHPatternSpec.swift · Heartbeat.swift  [C5]
  ...
  SentrySOC/                           ← the app target
    ...
    Sources/
      ...
      State/      GameModel.swift · SettingsState.swift · EffectRunner.swift
                  PlayFocus.swift (moved off `.shared` by P1-6)                        [C6, C5 per R9]
      Services/   SaveStore.swift · Flags.swift · SafariView.swift · QAJump.swift
                  ScreenRegistry.swift · Composition.swift · PlaceholderScreen.swift (B6)     [C6]
      ...
      Components/ SystemBar · ECGCanvas · Dock · SheetChrome · MeterView · StatTile · Chip
                  ...
                  InboxCard · RankBadge · CoachBubble · RichTextView · SegmentedTabs
                  Pressable · ComponentKit (the QA catalogue, `#if SENTRY_QA` — P1-9)         [C7]
      Screens/Play/  ShiftIntroView · CaseView · SourceSheet · EvidenceBoard · CallSheet
                     DebriefView · BoardSheet · AbandonSheet · ShiftSummaryView (B6)
                     PlayChrome · PlayComposition (B6's installer)                            [C8]
      Screens/Meta/  HubView · RankUpView · SettingsView (Licences is a NavigationStack page
                     inside it, not its own file) · FirstRunView · KitSheet · MetaChrome
                     MetaComposition (B6's installer) · MetaIdentifiers                       [C9]
      Haptics/    HapticsEngine.swift · CHPatterns.swift · HeartbeatPlayer.swift
                  SensoryCues.swift · HapticsHost.swift · HapticsComposition.swift (B6)
                  — six files as shipped (C11)                                                [C10]
    Resources/
      Assets.xcassets/   AppIcon · AccentColor · LaunchGround · LaunchMark                    [C6]
      IBMPlexMono-{Regular,Medium,SemiBold}.ttf                                               [C6]
      SpaceGrotesk-{Regular,Medium,Bold}.ttf   ← R8/R11: **Bold, not SemiBold** — Space
                  Grotesk ships no SemiBold static upstream (404). Six faces, all six called
                  by a step of Typography's scale; provenance in `Resources/FONTS.md`.        [C6]
      OFL-IBMPlex.txt · OFL-SpaceGrotesk.txt                                                  [C6]
    Tests/      FontRegistrationTests · SaveStoreTests · EffectScheduleTests · HubDockTests
                CareerResetTests · PhaseHostDismissTests (P1-1/2/3) · Components/*            [C6]
  scripts/      shots.sh · verify.sh · swiftlex.py + the three committed allowlists
                (s1-allow.txt · profanity-allow.txt · d1-base.txt). No `build.sh` — the
                Makefile is the build entry point; no `appicon.swift` — the icon is committed. [C11]

Makefile                                                                                     [C11]
.github/workflows/ios.yml                                                                    [C11]
docs/IOS-BUILD.md · docs/APPSTORE.md · docs/PLAYTEST-ios.md · docs/screenshots/ios/**        [C11]
docs/ios/SPEC.md · SPEC-ADDENDUM.md · DESIGN.md   (C11 edits; the lead commits the originals) [C11]
```
...
| `game/trace.ts` | `Engine/TraceOps.swift` | `status(_:)`, `clamp(_:)` — **R8**: the basename is `TraceOps`, because `Model/Trace.swift` is in the same target and a Swift target cannot hold two files of one name |
...
- **DV-6** `CareerState.dailyDoneOn` is iOS-only — the web tracks no daily completion.
- **DV-7** `inboxFor(features:)` has an `.iOS` mode: `redRunsDone` is treated as ≥ 1 for selection so
  the engine never emits `tip-redrun`, and two messages are re-voiced seat-neutral (**R1**).
- **DV-8 (R8)** `rankFor` **traps** on an empty ladder and `ContentPack.bundled` **traps** on a decode
  failure. Both are programmer errors, not runtime conditions: a bundle that will not decode cannot
  draw a screen, and a nil rank would spread `Optional` through every meta surface. The TypeScript
  returns `undefined` and lets the caller cope; Swift refuses to start. `applyCall` grades twice on
  purpose — literal parity with `SocConsole.tsx`, noted in a doc comment on `ShiftOps.swift`.
- **DV-9 (P1, §7.2)** `CareerState.clearedShiftIDs: Set<String>` is iOS-only, like `dailyDoneOn`.
  "Cleared" is **recorded**, not derived from standing; hand-written `Codable` `decodeIfPresent`s it
  and omits an empty set, so an old save still loads. Written in exactly one place,
  `settlement(for:career:content:now:)`, and for **campaign boards only** — a daily id carries its
  date, and recording those would put a year of dead strings in the save.
...
- **Abandon** and **Reset career** are `.confirmationDialog` — the system affordance for destructive
  ...
  **STALE — superseded by SPEC-ADDENDUM §7.3 (P1).** *Abandon is a **sheet***: `ViewID.abandon` is a
  view the reducer opens with no board under it, and `DESIGN.md` §2.1/§2.5 draw a sheet with two
  buttons in the thumb arc. *Reset career is an **`.alert`***: measured on iOS 26, a
  `confirmationDialog` presented **over a sheet** renders as the compact centred dialog and **drops
  the `.cancel`-role button entirely**, which is the one button a destructive confirmation must have.
...
**plus a fourth arm** (SPEC-ADDENDUM §7.3, P1-3): a player can clear every board their standing has
opened while boards above are still locked (a rough Shift 1 pays ⬢ 15, Shift 2 opens at ⬢ 40) and the
daily itself opens at ⬢ 40, so the third arm would offer a CTA the reducer refuses. The fourth arm
offers a **replay of the highest unlocked board**, which is what its queue row already calls
`cleared · replay`. The label and the tap are one pure function, `HubView.dockTarget(...)`, and
"cleared" is **recorded** in `CareerState.clearedShiftIDs` (DV-9), never derived from standing.
...
is a 13 pt text button placed deliberately **outside** the thumb arc behind the abandon **sheet**
(SPEC-ADDENDUM §7.3 — this line's original "`.confirmationDialog`" is stale; see §4.2).
...
**A phase, not a `.fullScreenCover`** (SPEC-ADDENDUM §7.3): `.milestone` is a phase and `PhaseHost`
renders phases into its `ZStack`; only `ViewID.firstRun` goes through a cover. The screen is
full-bleed by construction — it paints its own ground edge to edge, draws no `SystemBar`, has nothing
presented over it and no back gesture, which is what this section actually asks for.

No SystemBar — the one cinematic beat. `RankBadge` is a 6-point
...
targets:
  SentrySOC:
    ...
    settings:
      ...
      # SUPERSEDED by SPEC-ADDENDUM §6 (2026-09-05, after the founder's first device run):
      # `CODE_SIGNING_ALLOWED/REQUIRED: NO` is NOT set in project.yml. With signing off,
      # Xcode installs an unsigned binary on a physical iPhone and the device rejects it
      # (`LaunchExecutableValidationErrorDomain`). Signing is `Automatic` with
      # `DEVELOPMENT_TEAM: "${SENTRY_DEV_TEAM}"`; simulator, CI and release-guard builds pass
      # CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO on the **xcodebuild command line**
      # instead, so a fresh clone still builds with no Apple account. See ios/project.yml.
    info:
      ...
      properties:
        ...
        UIAppFonts:                              # BARE filenames — resources flatten (D21)
          ...
          - SpaceGrotesk-Bold.ttf                # R8/R11: **Bold**, not SemiBold — upstream
                                                 # ships no SpaceGrotesk-SemiBold static (404).
                                                 # Bold is the heavy human cut; all six faces
                                                 # are bound by a step of Typography's scale.
...
#   MEASURED 2026-09-06 on a clean clone (C11): **186 tests in 17 suites**, not ~250 —
#   the estimate below counted parameterised rows, `swift test` counts `@Test` cases.
#   96 golden grades + 12 synthetic + trace + 7 shift runs + scoring
...
#   The two flags are AMENDED per SPEC-ADDENDUM §6: they live on the command line, never in
#   project.yml, so the founder's device run still signs automatically.
...
#   Nine named checks; `sh ios/scripts/verify.sh <name>` runs one. As implemented (C11):
#   1 export     the committed export is not dirty vs a fresh run (`npm run soc:check`)
#   2 hash       contentHash differs across any of the 10 generated files
#   3 qa         `strings` on the **Release** binary contains SENTRY_QA / SentryQAScreen, OR the
#                **Debug** binary does not (the B7 positive control — "grep broken"). Measured on
#                Xcode 26.2: ENABLE_DEBUG_DYLIB=YES, so the Debug code is in
#                `SentrySOC.app/SentrySOC.debug.dylib` and the plain binary is a ~40 KB stub —
#                the guard reads the dylib for the control and the binary for the assertion.
#                Builds `-configuration Release -destination 'generic/platform=iOS Simulator'`
#                with CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO (SPEC-ADDENDUM §6).
#   4 fonts      any Resources/*.ttf has no matching OFL text
#   5 colours    a hex colour or a font name appears outside Design/ (comments excluded by ruling,
#                and reported as a notice)
#   6 s1         a string literal containing a letter is authored in Screens/ or Components/ —
#                S1, with `ios/scripts/s1-allow.txt` for addresses and diagnostics
#   7 profanity  grep -inE '\b(damn|shit|fuck|hostage|weapon)\b|\bkill(s|ed|ing)?\b' over
#                copy.json + content.json + the app's Swift hits outside
#                `ios/scripts/profanity-allow.txt` (the 4+ age-rating questionnaire answer, B3)
#   8 pay       the B4 regex over Swift **string literals** (R12 — never raw Swift, so `$0` and
#                `"\($0.min)"` cannot trip it) and over the exported JSON
#   9 d1        the merge-base diff touches next.config.ts open-next.config.ts wrangler.jsonc
#                app/api, or an iOS stage touched the protected web tree (S7; the web half
#                measures from `ios/scripts/d1-base.txt`, see that file for why)
#   DROPPED: `npm ls tailwindcss` — a Capacitor-era check on a web bundle that no longer exists.
```

**Makefile:** `make export` (1) · `make contract` (1) · `make engine` (2) · `make project` (3) ·
`make build` / `make test` / `make app` (3+4) · `make shots` (5) · `make guard` = `make verify` (6) ·
`make` = the whole gate in order.
...
3. **Run only your own targets.**
   ...
   - App tickets (C6–C10) run `xcodegen generate --spec ios/project.yml` then
     `xcodebuild build -project ios/SentrySOC.xcodeproj -scheme SentrySOC -destination 'platform=iOS Simulator,id=C2136147-45C8-42DD-8E3A-EDE974B97154' CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO`.
     Only C6 and C11 run `xcodebuild test`. **The two signing flags are mandatory on every
     command line** (SPEC-ADDENDUM §6, amended 2026-09-05) — `project.yml` keeps automatic signing so
     the founder's device run works, so a build without them needs a Team and fails on a bare clone.
