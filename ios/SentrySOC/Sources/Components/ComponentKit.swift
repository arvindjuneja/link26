...
/// **Compiled only under `SENTRY_QA`** (P1-9). This is a QA harness, not a screen:
/// nothing in the shipped app mounts it, its section headings are Swift type names
/// rather than copy, and its rows are filled with hand-picked counts (`3/6`, `86%`,
/// `⬢ 80`) that exist to exercise a layout rather than to state anything true about a
/// career. That is exactly right for a catalogue and exactly wrong for something a
/// player could ever reach, and S1's rule — no player-facing literal under
/// `Components/**` — cannot tell the two apart. The compilation condition can:
/// `project.yml` sets `SENTRY_QA` on Debug alone, so the harness is in every build a
/// reviewer or the snapshot suite runs, and in none that ships.
#if SENTRY_QA
  ...
  struct ComponentKitView: View {
    ...
    /// A pinned instant for the two live traces on this page, so the snapshot suite's
    /// PNGs do not change with the second they were taken (P1-9). `nil` on screen.
    var now: Date?
    ...
    init(
      ...
      blocks: [Block] = Block.allCases, now: Date? = nil
    ) {
      ...
      VStack(spacing: 0) {
        ...
        ForEach(TraceStatus.allCases, id: \.self) { status in
          SystemBar(
            ...
            trace: .init(
            status: status, label: bandLabel(status), bpm: content.tuning.bpm[status],
            now: now),
    ...
    private var dockHint: String {
      let findings = copy.plural("caseFindingsCount", 3)
      ...
      section(MeterView.self, ECGCanvas.self, StatTile.self) {
        ...
        ForEach(TraceStatus.allCases, id: \.self) { status in
          HStack(spacing: 12) {
            ...
            ECGCanvas(status: status, bpm: content.tuning.bpm[status], now: now)
              .frame(height: 24)
          }
        }
      }
